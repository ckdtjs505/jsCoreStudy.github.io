...
JS 대표 사이트인 MDN에서의 설명이다. 이해하기 어렵지만 천천히 이해해 보자. 어휘적 환경의 의미를 먼저 생각해보자. `어휘적 환경(LexicalEnvironment)`은 이전 컨텍스트 객체의 정보를 가리키고 있다. 이에따라 이전 컨텍스트에서 정의된 변수에 접근이 가능하게 된다.
